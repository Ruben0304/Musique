...
class SearchSongAdapter(private val context: Context) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SongViewHolder {
        val layoutId = R.layout.song_item_search
    ...
    inner class SongViewHolder(itemView: View, private val context: Context) :
...
//                        downloadService?.addDownloadTask(url, filePath)
                    ...
                    val metadataDownloader = MetadataDownloader(
                        context,
                        FetchMetadata(), MetadataSaver(context)
                    )
                    metadataDownloader.download(song.id)
